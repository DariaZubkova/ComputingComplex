    ...
    def draw_tol_plot():
        ...
        for i in range(x.shape[0]):
            for j in range(x.shape[0]):
                vec = numpy.array([[x[i]], [y[j]]])
                z[i, j] = calcfg(vec)[0]

        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        surf = ax.plot_surface(x, y, z, cmap=cm.BrBG, shade=True)
        ax.plot([3.2287823e-16], [3.2287823e-16], [z[399, 201]], markerfacecolor='c', markeredgecolor='c', marker='*',
                markersize=10)
        print('here!')
        print(index(z, max(z)))
        print(z[399, 201])
